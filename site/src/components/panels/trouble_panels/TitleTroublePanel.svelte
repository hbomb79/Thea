<script lang="ts">
    ...
    let currentResolver = "";
    const validResolvers = [["Manual", "struct"]];
...
{#if currentResolver == "struct"}
    ...
    <p>
        ...
        <br /><br />
        Fill out the form below to resolve this problem,
        <i>you can leave a field blank if it's not applicable (e.g. episode number for a movie)</i>
    </p>

    <DynamicForm on:submitted={(event) => resolveTitle(event.detail)} fields={queueDetails.trouble.expected_args} />
{/if}

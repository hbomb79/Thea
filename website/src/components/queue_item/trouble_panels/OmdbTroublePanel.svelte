...
{#if currentResolver == "choice"}
    <h2>Choose Option</h2>
    <p class="trouble">
        Our search through OMDB resulted in multiple options. Pick the correct one below.<br />If none are correct, you
        can provide an IMDB id or provide the item details manually using the navigation above.
    </p>
...
{:else if currentResolver == "struct"}
    ...
    <p>If this item doesn't exist in OMDBs database yet, you can instead provide all the details manually below.</p>
    <DynamicForm
        fields={troubleDetails.expected_args}
        on:submitted={(event) => resolveWithForm({ replacementStruct: event.detail })}
    />
{/if}
